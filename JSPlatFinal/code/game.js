...
ZeroGravity.prototype.type = "zerogravity";
function elt(name, className) {
	var elt = document.createElement(name);
	if(className) elt.className = className;
	return elt;
...
ZeroGravity.prototype.act = function(step, level) {
...
Enemy.prototype.act = function(step, level) {
	...
	this.pos = newPos;
	if(Math.round(newPos.x) % counter == 0 && this.speed.x == 2)
		this.speed.x = -2
	else if(Math.round(newPos.x) % counter == 0 && this.speed.x == -2)
		this.speed.x = 2
	//console.log(Math.round(newPos.x))
};
